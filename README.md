...
functions, use the functions below.  
  ...
  - `mojchart_palette()` returns a colour palette with up to six
    colours.  
  - `moj_colours()` provides names and hex codes for MoJ corporate
    colours.  
  - `palette_colours()` provides names and hex codes for all colours
    used in the mojchart palettes.  

See the function help files for details and examples.
...
shading areas, such as for bar charts, since they are easier on the eye.
The vibrant schemes are recommended for lines or points, where vibrant
colours are easier to distinguish. The vibrant schemes use colours taken
directly from MoJ corporate branding, whereas the muted colours are
modified versions with reduced saturation.
...
``` r
library(mojchart)
pal <- mojchart_palette(n = 6, scheme = "muted3")
swatch(pal)
```
