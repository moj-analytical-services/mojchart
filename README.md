...
To preview palettes:
  ...
  - `multiswatch()` displays all the palettes comprising a specified
    colour scheme.

See the function help files for details and examples for all the package
functions.
...
subtitle for additional statistical details. The data source can be
shown in the caption below the chart. Illustration charts are shown in
the examples section below.

Axis labels should be horizontal according to both the GSS guidance and
...
can instead be included within the ggplot2 subtitle if needed. This is
demonstrated by the second of the example charts.

`theme_gss()` includes arguments to show or hide the x axis label and x
axis tick marks. Setting the argument `flipped = TRUE` reverses the x
...
recommended for lines or points. The muted schemes are intended for
shading areas, such as for bar charts, in line with the [GSS guidance on
the use of
...
colours. Consider the accessibility implications if you need more
colours than that (see below).
...
To return a palette as a vector, use the `mojchart_palette()` function,
with the number of colours needed and the name of a colour scheme. The
`swatch()` function can be used to show a preview.
...
`scale_colour_moj()` and `scale_fill_moj()` can be used to apply a
palette to a ggplot2 chart, as shown in the examples section below.
